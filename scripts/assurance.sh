...
# `read_lines NAME < <(cmd)` — fill array NAME with cmd's non-empty lines.
# Stands in for `mapfile -t`, which needs bash 4; macOS still ships bash 3.2 and
# this script is meant to run locally as well as on the Ubuntu runner.
read_lines() {
...
read_lines INFERRED < <(kane-cli context list --type usecase --inferred --json 2>/dev/null | jq -r '.id' || true)
...
read_lines USECASES < <(kane-cli context list --type usecase --json 2>/dev/null | jq -r '.id' || true)
...
if [ "$FORCE_DESIGN" != "true" ]; then
  DESIGN_STATUS_TSV=$(kane-cli cover gaps --stage design --json 2>/dev/null \
    | jq -r '.usecases[]? | [.id, (.design_completeness.status // ""), ((.stale_acs // 0) | tostring)] | @tsv' 2>/dev/null \
    || true)
  if [ -z "$DESIGN_STATUS_TSV" ]; then
...
# Column 2 (status) and column 3 (stale AC count) of the row for a use-case id.
design_status() {
  printf '%s\n' "$DESIGN_STATUS_TSV" | awk -F'\t' -v id="$1" '$1 == id { print $2; exit }'
}
stale_acs_for() {
  printf '%s\n' "$DESIGN_STATUS_TSV" | awk -F'\t' -v id="$1" '$1 == id { print $3; exit }'
}
...
read_lines GRAPH_TESTS < <(graph_test_ids)
ON_DISK=" $(test_ids_in .testmuai/tests | tr '\n' ' ') "
MISSING=()
# ${ARR[@]} on an empty array trips `set -u` on bash 3.2, so guard the loop.
for t in ${GRAPH_TESTS[@]+"${GRAPH_TESTS[@]}"}; do
  [[ "$ON_DISK" == *" ${t} "* ]] || MISSING+=("$t")
done
